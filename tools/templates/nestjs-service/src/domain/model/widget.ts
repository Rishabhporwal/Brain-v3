...
export class Widget {
  ...
  static create(input: { id: WidgetId; brandId: BrandId; name: string; priceMinor: number; now: Date }): Widget {
    ...
    const w = new Widget(input.id, input.brandId, input.name.trim(), Math.max(0, Math.trunc(input.priceMinor)), false, input.now)
  ...
  static rehydrate(row: { id: WidgetId; brandId: BrandId; name: string; priceMinor: number; archived: boolean; createdAt: Date }): Widget {
  ...
  get priceMinor() { return this._priceMinor }
