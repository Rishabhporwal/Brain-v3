...
/**
 * Service-to-service auth: callers present `x-internal-token` matching LEDGER_TOKEN.
...
export class InternalTokenGuard implements CanActivate {
  canActivate(ctx: ExecutionContext): boolean {
    ...
    if (!expected) {
      if (process.env.NODE_ENV === 'production') throw new UnauthorizedException('LEDGER_TOKEN not configured')
