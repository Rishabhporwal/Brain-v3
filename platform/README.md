...
- `billing/` — fee preview (BRD §23): realized-GMV-% above per-tier minimum, CM2 cap,
  Day-0–14 activation waiver; consumes realized GMV from the metric engine (never computes
  its own numbers); estimated revenue is never a billing basis. Security co-sign (CODEOWNERS).
- `api-gateway-bff/` — the single sync edge (Keycloak JWT verification, console
...
**Planned services** (auth, brand, membership, organization, onboarding, rbac, audit,
