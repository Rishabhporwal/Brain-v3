...
/**
 * Service-to-service auth: callers present `x-internal-token` matching BILLING_TOKEN.
...
export class InternalTokenGuard implements CanActivate {
  canActivate(ctx: ExecutionContext): boolean {
    const expected = process.env.BILLING_TOKEN
    if (!expected) {
      if (process.env.NODE_ENV === 'production') throw new UnauthorizedException('BILLING_TOKEN not configured')
