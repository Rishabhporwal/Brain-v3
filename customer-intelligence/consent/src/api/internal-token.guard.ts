...
/**
 * Service-to-service auth: callers present `x-internal-token` matching CONSENT_TOKEN.
...
export class InternalTokenGuard implements CanActivate {
  canActivate(ctx: ExecutionContext): boolean {
    const expected = process.env.CONSENT_TOKEN
    if (!expected) {
      if (process.env.NODE_ENV === 'production') throw new UnauthorizedException('CONSENT_TOKEN not configured')
